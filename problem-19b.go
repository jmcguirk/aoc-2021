...
func (this *Problem19B) Solve() {
	...
	file, err = os.Open("source-data/input-day-19b.txt");
	if err != nil {
		Log.FatalError(err);
	}
	defer file.Close()
	...
	for i, v1 := range solvedScanners{
		for j, v2 := range solvedScanners{
			if(i == j){
				continue;
			}
			dist := v1.Pos.Manhattan(v2.Pos);
	...
	Log.Info("Peak distance was %d", peakDistance)
...
func (this *Problem19B) FindBestOverlapAndShiftGivenRotation(referencePoints []*IntVec3, rotatedPoints []*IntVec3) (*IntVec3, int){
...
func (this *Problem19B) FindBestMatrix(solvedScanner *ProbeScanner, scannerInQuestion *ProbeScanner) (*M3, *IntVec3, int){
